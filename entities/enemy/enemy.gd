...
@onready var alert_sprite_2d: Sprite2D = $AlertSprite2D
...
func _ready():
	...
	alert_sprite_2d.visible = true
	alert_sprite_2d.scale = Vector2.ZERO
...
func state_normal():
	if is_multiplayer_authority():
		...
		var can_attack := attack_cool_down_timer.is_stopped() || global_position.distance_to(target_position) < 20
...
func enter_state_charge_attack():
	...
	alert_tween.tween_property(alert_sprite_2d,"scale",Vector2.ONE,.2)\
		.from(Vector2.ZERO)\
		.set_ease(Tween.EASE_OUT)\
...
func leave_state_charge_attack():
	...
	alert_tween = create_tween()
	alert_tween.tween_property(alert_sprite_2d,"scale",Vector2.ZERO,.2)\
		.from(Vector2.ONE)\
...
func acquire_target():
	...
	for player in players:
		if nearest_player == null:
			...
			nearest_squared_distance = player.global_position\
